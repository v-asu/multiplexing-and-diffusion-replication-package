...
## Plot village multiplexing against threshold-averaged complex path length.

input_path <- "data/processed/rct_villages/rct_village_union_network_mechanisms.csv"
output_path <- "figures/fig_si_09_multiplexing_complex_path_length.pdf"
...
main <- function() {
  plot_df <- read.csv(input_path) %>%
    as_tibble() %>%
    filter(!is.na(M_i), !is.na(plc))

  plot_plc <- ggplot(plot_df, aes(x = plc, y = M_i)) +
    geom_point(size = 3, alpha = 0.7, color = "#333333") +
    geom_smooth(method = "lm", se = TRUE, color = "black", fill = "gray90", linewidth = 0.8) +
    labs(
      x = "Complex Path Length (Union Layer)",
      y = "Multiplexing Index",
      title = " "
    ) +
    plot_theme

  ggsave(output_path, plot_plc, width = 7, height = 5, dpi = 300)
  message("Saved village multiplexing vs complex path length figure.")
}
